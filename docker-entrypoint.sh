...
#
# Dispatch the single vgi-mask image into one of its transports:
...
#
# The worker is stateless (pure masking engine, key passed per call), so there
...
case "${1:-http}" in
  http)
    ...
    exec mask-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec mask-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec mask-worker "$@"
