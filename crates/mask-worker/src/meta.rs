...
const SOURCE_BASE: &str = "https://github.com/Query-farm/vgi-mask/blob/main/crates/mask-worker/src";

/// Build the implementation `vgi.source_url` for a file under `mask-worker/src`,
/// e.g. `source_url("scalar/fpe.rs")`.
...
///
/// `relative_path` is the implementing file relative to `mask-worker/src`.
pub fn object_tags(
    title: &str,
    doc_llm: &str,
    doc_md: &str,
