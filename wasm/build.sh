#!/usr/bin/env bash
# Build the browser (`worker:`) build of the mask VGI worker: mask-wasm compiled
# to wasm32-unknown-emscripten, linked by emcc with the SAB ring js-library into
# a MODULARIZE'd Web Worker module.
#
...
LIB="target/$TARGET/release/libmask_wasm.a"
...
# which nested Arrow/crypto encoders can overflow (surfacing as a bare "memory
# access out of bounds" with no stack trace). INITIAL_MEMORY must cover the main
# stack + every pthread stack up front so startup does not stall growing shared
