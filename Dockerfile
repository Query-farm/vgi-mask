...
#
# Single image that serves every network transport of the `mask` VGI worker:
...
#
# Like vgi-units this worker is STATELESS: the masking engine (FF1 FPE over
# AES-256, HMAC-SHA-256 tokenization, redaction) is pure compute compiled into
# the binary and simply takes a key per call — there is no /data volume, no
...
# runtime ships. The `vgi` SDK and all crypto crates resolve from crates.io
# (no git patch dependency), so a plain `cargo build --locked` suffices.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin mask-worker \
    && cp target/release/mask-worker /mask-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-mask
...
LABEL org.opencontainers.image.title="vgi-mask" \
      org.opencontainers.image.description="Reversible format-preserving encryption, deterministic tokenization & irreversible redaction of sensitive values as a VGI worker for DuckDB/SQL (stdio + HTTP + TCP)" \
...
ENV PORT=8000 \
    ...
    VGI_MASK_GIT_COMMIT=${GIT_COMMIT}
...
COPY --from=build --chmod=0755 /mask-worker /usr/local/bin/mask-worker
