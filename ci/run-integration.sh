...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-mask
...
#               If VGI_MASK_WORKER is ALREADY set to an http(s):// URL (e.g. a
...
case "$TRANSPORT" in
  ...
  http)
    ...
    # image_test): if VGI_MASK_WORKER already points at an http(s) URL, use it
    # and skip spawning a local binary. The awk preprocessor still injects
    # httpfs because TRANSPORT=http.
    if [[ "${VGI_MASK_WORKER:-}" =~ ^https?:// ]]; then
      echo "Using pre-launched HTTP worker at $VGI_MASK_WORKER"
    else
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_MASK_WORKER, pattern: $TEST_PATTERN) ..."
