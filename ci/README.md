# CI: the vgi-mask worker integration suite
...
1. **Build the worker** — `cargo build --release --bin mask-worker`. The
   compiled `target/release/mask-worker` is a self-contained stdio worker the
   extension spawns (the `.test` files `ATTACH` it via `${VGI_MASK_WORKER}`).
